...
const Unit6 = () => {
    ...
    const { unit6 } = homeContent[locale] ?? homeContent.en;

    return (
        <div ref={heroRef} className="relative mt-[96px] h-[603px] bg-[url(/img/bg4.jpg)] bg-size-[100%_100%] mb-[24px] mobile:mt-[40px] mobile:h-[480px] mobile:bg-size-[auto_100%] mobile:bg-center rounded-[8px] mobile:mb-[16px] overflow-hidden">
            <div className="pl-[145px] pr-[178px] pt-[96px] mobile:px-[16px] mobile:pt-[40px] mobile:pb-[40px]">
                <div className="flex">
                    <img src="/img/icon17.svg" className="size-[15px] mobile:size-[12px]" alt="" />
                    <div className="text-[#F7F7F7] text-[12px] font-medium ml-[9px] tracking-[0.72px] mobile:text-[11px] mobile:ml-[6px]">
                        {unit6.badge}
                    </div>
                </div>
                <div className="pt-[35px] flex justify-between mobile:pt-[24px] mobile:flex-col mobile:gap-y-[24px]">
                    <div className=" h-[165px] relative items-center flex mobile:h-auto">
                        <div className="text-[72px] font-medium text-[#F7F7F7] leading-[82px] tracking-[-1.2px] mobile:text-[36px] mobile:leading-[42px] mobile:tracking-[-0.72px]">
                            {unit6.headingLines[0]}
                            <br />
                            <span className="text-[#FE3D11]">{unit6.headingLines[1]}</span>&nbsp;
                            {unit6.headingLines[2]}
                        </div>
                    </div>
                    <div className="w-[349px] mt-[6px] mobile:w-full mobile:mt-0">
                        {unit6.paragraphs.map((paragraph, index) => (
                            <div
                                key={paragraph}
                                className={`text-[#F7F7F7] text-[15px] ${index === 0 ? 'opacity-70' : ''} leading-[144%] tracking-[-0.24px] ${
                                    index === 0 ? '' : 'mt-[32px] mobile:mt-[20px] w-[400px]'
                                } mobile:text-[14px]`}
                            >
                                {paragraph}
                            </div>
            ...
            <div ref={cardRef} className=" absolute left-[115px] bottom-[-72px] w-[669px] h-[288px] p-[12px] rounded-[12px] outline-1 outline-offset-1 outline-[rgba(220,218,217,0.60)] bg-linear-to-b from-[rgba(247,247,247,0.18)] to-[rgba(247,247,247,0.09)] mobile:left-[5%] mobile:w-[90%] mobile:h-[240px]">
                <div className="self-stretch flex-1 p-[24px] size-full rounded-[12px] outline-1 outline-offset-1 outline-[rgba(220,218,217,0.48)] flex flex-col justify-start items-start gap-2.5 overflow-hidden">
                    <div data-property-1="Default" className="max-w-full cursor-pointer w-[597px] h-[96px] pl-[33.6px] pr-[38.4px] py-[38.4px] bg-linear-to-l from-[rgba(247,247,247,0.12)] to-[rgba(247, 247, 247, 0.00)] rounded-[9.60px] shadow-[0px_1.440000057220459px_2.880000114440918px_0px_rgba(0,0,0,0.12)] shadow-[inset_0px_-1.440000057220459px_2.1600000858306885px_0px_rgba(0,0,0,0.12)] shadow-[inset_0px_-0.24000000953674316px_0.9600000381469727px_0px_rgba(255,255,255,1.00)] outline outline-[0.80px] outline-offset-[-0.80px] outline-neutral-100/50 inline-flex justify-start items-center gap-[30px] group transition-all duration-200 hover:from-[rgba(247,247,247,0.18)] hover:to-[rgba(247,247,247,0.06)] hover:outline-[rgba(220,218,217,0.80)]">
                        ...
                        <a href="/pages/community" target="_blank" className="translate-y-[2px] text-center justify-start text-[#F7F7F7] text-[36px] font-medium leading-[106%] tracking-[-0.72px] mt-[2px] mobile:text-[24px]">{unit6.ctaLabel}</a>
                    </div>
                </div>
            </div>
        </div>
